<template>
    <f7-page @page:afterin="onPageAfterIn">
        <f7-navbar>
            <f7-nav-left :class="{ 'disabled': merging }" :back-link="tt('Back')"></f7-nav-left>
            <f7-nav-title :title="tt('Merge into another account')"></f7-nav-title>
            <f7-nav-right :class="{ 'disabled': merging }">
                <f7-link icon-f7="checkmark_alt"
                         :class="{ 'disabled': !fromAccount || !toAccountId || fromAccount?.id === toAccountId || !toAccountName || !isToAccountNameValid || merging }"
        ...
        <f7-card class="margin-top-half"
            ...
            :content="tt('format.misc.mergeAccountTip', { fromAccount: fromAccount?.name, toAccount: displayToAccountName })">
        </f7-card>
        ...
        <f7-list form strong inset dividers class="margin-vertical" v-if="!loading">
            <f7-list-item
                ...
                link="#" no-chevron
                :class="{ 'disabled': merging || !allVisibleAccounts.length }"
...
function confirm(): void {
    ...
    transactionsStore.mergeAccounts({
        ...
        showToast('Accounts have been merged.');
        router.back();
    }).catch(error => {
        merging.value = false;
        hideLoading();
